        ...
        <div class="content-wrapper">
          <div class="row">
            ...
            <div class="col-md-6 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Edit writter form</h4>
                  <?php
                    require_once '../../includes/dbh.inc.php';
                    $id = $_GET['id'];
                    $sqlca = "select * from `writter` where `writter_id`='$id'";
                    $res = mysqli_query($conn, $sqlca) or die('quey failed');
                    $relc = mysqli_fetch_assoc($res);
                    ?>
                  <form action="../../includes/update-writter.inc.php?id=<?= $relc['writter_id'] ?>" method="post"  enctype="multipart/form-data" class="forms-sample">
                    <?php
                     if (isset($_GET['msg'])) {
                    ...
                    <div class="form-group">
                      <label for="exampleInputName1">writter Name</label>
                      <input type="text" class="form-control" id="exampleInputName1" placeholder="writter_name" name="writter_name" value="<?=$relc['writter_name']?>"/>
                    </div>
                      <!-- ================ -->
